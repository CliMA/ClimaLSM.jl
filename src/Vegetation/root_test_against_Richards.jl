...
function roots(dy,y,paramset2,t)
    ...
    for i in 1:m-1
        flux_stem[i+1] = flux(z2[i], z2[i+1], p[i], p[i+1], a, a, b, b, Ksat, Ksat, S_l[i])
    end
...
function roots_Yujie(dy,y,paramset2,t)
    ...
    for j in 1:m+1
        dy[j] = flux_stem[j] - flux_stem[j+1]
    end   
end
...
##### Re against plant hydraulics model
RE_against_PHM = plot(z,truth.u[1],label="t0",xlabel="Stem height [m]",ylabel="Augmented liquid fraction in stem [m3 m-3]",legend=:bottomleft,dpi=300)
plot!(RE_against_PHM, z,truth.u[end],label="T_end_finely_resolved_RE")
plot!(RE_against_PHM, z,ν_vec,label="porosity")

# Plant hydraulics model
a = FT(0.3968) # modified logistic fitted to VG
b = FT(7.692)
...
#=
num_compartments = Array([2]) #,10,20,40,55])
absolute_error_as_function_of_height=plot()

for i in 1:length(num_compartments)
m = num_compartments[i]
Δz2 = (zmax-zmin)/m
z2 = Array((zmin+Δz2/FT(2.0)):Δz2:(zmax-Δz2/FT(2.0)))
z3 = Array(zmin:Δz2:zmax)

# Solve the problem
paramset2 = [ν, vg_α, vg_n, vg_m, Ksat, S_s, θ_r, top_flux_bc, bot_flux_bc, a, b, m, z2, z3]
@time begin
sol = solve(ODEProblem(roots,θ0,tspan,paramset2),alg,adaptive=false,dt=dt)
end
...
spl = Spline1D(z2, theta_end, k=k)
theta_interpolated(zs) = spl(zs)
A = [theta_interpolated(zs) for zs in z2]
interp_linear = LinearInterpolation(z2, A)
interp_linear_extrap = LinearInterpolation(z2, A,extrapolation_bc=Line()) 
interp_theta_at_zmin=interp_linear_extrap(z[1]) # outside grid: linear extrapolation
interp_theta_at_zmax=interp_linear_extrap(z[end]) # outside grid: linear extrapolation
z4 = [z[1]; z2; z[end]]
theta_end = [interp_theta_at_zmin; theta_end; interp_theta_at_zmax]

plot!(RE_against_PHM,z4,theta_end, marker=(:circle,1),label=["tend_plant_hydraulics, n =", m], dpi=300)
#plot!(z2,theta_start,label="t0_plant_hydraulics")
savefig(RE_against_PHM,"root_test_against_Richards.png") 

# Error as a function of height
theta_end_at_z = interp_linear_extrap(z)
abs_error = truth.u[end] - theta_end_at_z
plot!(absolute_error_as_function_of_height,z,abs_error, label=["n =", m],legend=:bottomleft,dpi=300)
end
savefig(absolute_error_as_function_of_height,"absolute_error_as_function_of_height.png")
=#
...
###### Plot 1: Model vs truth as function of time, over 6 hours
...
let k=0
    for i in Int64(1):Int64(hour/dt):Int64(7*hour/dt) 
        k=k+1
        plot!(RE_against_PHM_6h,z,truth.u[i],line=(:dash, 1),label=label_truth[k], legendfontsize=10, color=RGB((k+2)/10, 0.3, 0.4))
        plot!(RE_against_PHM_6h,z2,theta[:,i],line=(:dot, 1),label=label_model[k],color=RGB((k+2)/10, 0.3, 0.4),legend=:bottomleft,dpi=300,xlabel="Stem height [m]",ylabel="Augmented liquid fraction in stem [m3 m-3]")
    end
end
plot!(RE_against_PHM_6h,z,ν_vec,label="porosity")
#annotate!(RE_against_PHM_6h, 0.1, 0.4910, text("full line: truth", :left, 10))
#annotate!(RE_against_PHM_6h, 0.1, 0.4900, text("dashed line: model with 60 compartments", :left, 10))
savefig(RE_against_PHM_6h,"RE_against_PHM_6h_60_compartments.png")

###### Plot 2: Model vs truth as function of time, over 20 days
...
let k=0
    for i in Int64(day/dt):Int64(5*day/dt):Int64(25*day/dt) 
        k=k+1
        plot!(RE_against_PHM_20_days,z,truth.u[i],line=(:dash, 1),label=label_truth[k], legendfontsize=10, color=RGB((k+2)/10, 0.3, 0.4))
        plot!(RE_against_PHM_20_days,z2,theta[:,i],line=(:dot, 1),label=label_model[k],color=RGB((k+2)/10, 0.3, 0.4),legend=:bottomleft,dpi=300,xlabel="Stem height [m]",ylabel="Augmented liquid fraction in stem [m3 m-3]")
    end
end
plot!(RE_against_PHM_20_days,z,ν_vec,label="porosity")
#annotate!(RE_against_PHM_20_days, 0.1, 0.4910, text("full line: truth", :left, 10))
#annotate!(RE_against_PHM_20_days, 0.1, 0.4900, text("dashed line: model with 60 compartments", :left, 10))
savefig(RE_against_PHM_20_days,"RE_against_PHM_20_days_60_compartments.png")
