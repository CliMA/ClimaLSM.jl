...
precip_θ_T = readdlm("/Users/katherinedeck/Desktop/ozark_site/p_data_2005.csv",',')
...
transpiration_function(t::ft) where {ft} = et_spline(t) > 0.0 ? et_spline(t) : 0.0
...
ic = [0.402992, 0.40231, 0.401632, 0.400957, 0.400285, 0.399616, 0.398951, 0.398289, 0.39763, 0.396974]
...
#plots
data = readdlm("/Users/katherinedeck/Downloads/Re _plant_and_soil_hydraulics_benchmark/holtzman_clima_output.csv", ',')
lwp = data[2:end,20]
dates = data[2:end,1]
...
lwp_leaf = [Roots.θ_to_p.(sol.u[k].vegetation.θ[2])./1e6 for k in 1:1:length(sol.t)];

begin
    default(titlefont = (2, "times"), legendfontsize = 2, guidefont = (2, :black), tickfont = (2, :black), guide = "x", framestyle = :zerolines, yminorgrid = true)
    ...
    plot!(sol.t./3600 ./ 24, ϕ_leaf./1e6, label="ϕ leaf, MPa", xlabel = "t(days since Jan 1)", ylabel = "ϕ(MPa)")
    plot!(xlim = [0, maximum(sol.t) ./3600 ./24])
    
    plot!(legend = :bottomright)
    
    plot2 = plot(sol.t./3600 ./24, [parent(sol.u[k].soil.ϑ_l)[end] for k in 1:1:length(sol.t)], label="10cm", xtickfontsize = 5, ytickfontsize = 5)
    plot!(sol.t./3600 ./24, [parent(sol.u[k].soil.ϑ_l)[end-1] for k in 1:1:length(sol.t)], label="30cm")
    plot!(sol.t./3600 ./24, [parent(sol.u[k].soil.ϑ_l)[end-2] for k in 1:1:length(sol.t)], label="50cm")
    plot!(sol.t./3600 ./24, [parent(sol.u[k].soil.ϑ_l)[end-3] for k in 1:1:length(sol.t)], label="70cm")
    plot!(sol.t./3600 ./24, [mean(parent(sol.u[k].soil.ϑ_l)) for k in 1:1:length(sol.t)], label="mean")
    plot!(legend = :topright,xlabel = "t (days)", ylabel = "S soil")
    plot!(ylim = [0.35,0.8])
    
    
    ϕ_soil = [parent(sv.saveval[k].soil.ψ .+ cds.soil.z) .* 9800 for k in 1:1:N_days];
    pl = scatter() 
    function plot_each_day(pl,indices)
        for i in indices
            plot!(pl, ϕ_soil[i][:]./1e6, parent(cds.soil.z), label = "")
        end
        plot!(pl, xlabel = "ϕ_soil (MPa)", ylabel = "z(m)", legend = :bottomright)
end
